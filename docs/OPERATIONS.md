...
- 열람 권한: **로그인 필수**. 어썸 리더는 샘플까지 포함해 회원 전용이며, 비로그인 접근은 `/auth/login?next=…`로 돌려보냅니다
- 가입: **초대 쿠폰 전용**. 트라이얼 기간이라 관리자(contact@awesomebooks.asia)에게 메일로 쿠폰을 요청해야 가입할 수 있습니다
- 구독료: **무료 샘플 구독**. 서가 30권까지 무료, 100권까지 무료 샘플 구독으로 운영 (사이트에 명시됨)
- 아마존 계약: 신간은 출간 후 90일간 아마존 독점이라, 어썸 리더에는 90일 뒤에 올라옵니다 (사이트에 명시됨)
- 유료 구독: 결제 코드는 준비 완료·휴면 상태
...
3. **Google 로그인** (선택): Providers → Google에 OAuth 클라이언트 ID/Secret 등록. 등록 전에는 오류만 나므로 로그인/가입 화면에서 Google 버튼을 숨겨 두었습니다 — 등록을 마치면 Vercel 환경변수에 `NEXT_PUBLIC_GOOGLE_AUTH=1`을 추가하고 재배포하면 버튼이 다시 나타납니다

### 받은 편지 (문의 · 편지 신청)
홈페이지의 편지 띠와 `/ko/contact`에 남긴 내용은 모두 `public.inbox` 테이블에 저장되고, 콘솔의 **받은 편지**에서 봅니다. 주소를 클릭하면 바로 답장이 열리고, 오른쪽 위 버튼으로 편지 신청자 전체를 BCC에 넣은 메일을 한 번에 열 수 있습니다. 읽기는 관리자만 가능하고, 쓰기는 누구나 가능합니다(문의 폼이니까).
**자동으로 contact@awesomebooks.asia로 전달되지는 않습니다** — 메일 발송 서비스가 아직 없습니다. Resend API 키를 등록하면 새 글이 올 때 그 주소로 알림을 보내도록 붙일 수 있습니다.
...
관리자 화면은 사이트와 분리된 **관리자 모드**입니다 — 상단 메뉴의 “관리자 모드” 버튼으로 들어가고, 콘솔 오른쪽 위 “사이트로 나가기”로 빠져나옵니다. 이 버튼은 관리자에게만 보입니다.
