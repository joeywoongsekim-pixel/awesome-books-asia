...
export default function Reader({
  ...
  /* One book, open. It used to be a desk: this book plus three more on a
     shelf beside it, draggable down and back. That is a lovely thing to
     look at and the wrong thing to read on — what a reader wants when
     they open a book is the book. */
  const [spread, setSpread] = useState(initialSpread ?? 0);
  ...
  const main: Book = BOOKS[initialIndex];
  const left = spread * 2;
  const right = left + 1;
  ...
  return (
    <div className={`rd${focus ? ' focus' : ''}`} data-noprint={t('noPrint')}>
      ...
      {/* The panel still speaks of a desk; with one book on it, the desk
          is that book. */}
      <AiPanel open={aiOpen} order={[initialIndex]} spreadOf={{[initialIndex]: spread}} />
